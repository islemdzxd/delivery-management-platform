...
import { incidentAPI, expeditionAPI } from '../services/api';
import { FaPlus, FaEdit, FaArrowLeft } from 'react-icons/fa';

const IncidentManagement = ({ onBack }) => {
  ...
  const statusOptions = [
    ...
    { value: 'CLOS', label: 'Clos', color: 'gray' },
  ];
  ...
  const resetForm = () => {
    setFormData({ expedition: '', type_incident: 'RETARD', description: '', statut: 'OUVERT' });
    setShowForm(false);
  };
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Gestion des Incidents</h2>
          <p className="text-gray-600 mt-1">Gérer tous les incidents avec opérations CRUD complètes</p>
        </div>
        <button
        ...
        >
          <FaPlus /> {showForm ? 'Annuler' : 'Nouvel Incident'}
        </button>
      </div>

      {showForm && (
        <div className="bg-white rounded-lg shadow-lg p-8 border border-gray-200">
          <h3 className="text-2xl font-semibold mb-6 text-gray-900">Nouvel Incident</h3>
            ...
            <tbody className="divide-y divide-gray-200">
              {incidents.map((incident) => (
                <tr key={incident.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4">{new Date(incident.date_incident).toLocaleDateString('fr-FR')}</td>
                  <td className="px-6 py-4">{typeOptions.find(t => t.value === incident.type_incident)?.label}</td>
                  <td className="px-6 py-4">{incident.description}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs font-semibold rounded-full bg-${statusOptions.find(s => s.value === incident.statut)?.color}-100`}>
                      {statusOptions.find(s => s.value === incident.statut)?.label}
                    </span>
                  </td>
                </tr>
              ))}
